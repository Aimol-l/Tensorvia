...
void SubImpl<Device::CUDA>::execute(const Tensor& a,const Tensor& b,Tensor& dst){
    DataType res_type = compute_type(a.dtype(),b.dtype());
    if(dst.dtype() != res_type){
        throw std::runtime_error("dst dtype error!");
    }
    ...
    if(a.dtype() == b.dtype()){
        dispatch_dtype(a.dtype(), [&](auto type_id) {
            using T = typename decltype(type_id)::type;
            if constexpr(std::is_same_v<T,float16>){
                sub_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __half*>(a.data()),static_cast<const __half*>(b.data()), static_cast<__half*>(dst.data()), a.numel());
            }else if constexpr(std::is_same_v<T,bfloat16>){
                sub_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __nv_bfloat16*>(a.data()),static_cast<const __nv_bfloat16*>(b.data()), static_cast<__nv_bfloat16*>(dst.data()), a.numel());
            }else{
                sub_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const T*>(a.data()),static_cast<const T*>(b.data()), static_cast<T*>(dst.data()), a.numel());
            }
        });
        ctx_impl->wait(); 
        return;
...
void DotImpl<Device::CUDA>::execute(const Tensor& a,const Tensor& b,Tensor& dst){
    DataType res_type = compute_type(a.dtype(),b.dtype());
    if(dst.dtype() != res_type){
        throw std::runtime_error("dst dtype error!");
    }
    ...
    if(a.dtype() == b.dtype()){
        dispatch_dtype(a.dtype(), [&](auto type_id) {
            using T = typename decltype(type_id)::type;
            if constexpr(std::is_same_v<T,float16>){
                dot_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __half*>(a.data()),static_cast<const __half*>(b.data()), static_cast<__half*>(dst.data()), a.numel());
            }else if constexpr(std::is_same_v<T,bfloat16>){
                dot_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __nv_bfloat16*>(a.data()),static_cast<const __nv_bfloat16*>(b.data()), static_cast<__nv_bfloat16*>(dst.data()), a.numel());
            }else{
                dot_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const T*>(a.data()),static_cast<const T*>(b.data()), static_cast<T*>(dst.data()), a.numel());
            }
        });
        ctx_impl->wait(); 
        return;
...
void DivImpl<Device::CUDA>::execute(const Tensor& a,const Tensor& b,Tensor& dst){
    DataType res_type = compute_type(a.dtype(),b.dtype());
    if(dst.dtype() != res_type){
        throw std::runtime_error("dst dtype error!");
    }
    ...
    if(a.dtype() == b.dtype()){
        dispatch_dtype(a.dtype(), [&](auto type_id) {
            using T = typename decltype(type_id)::type;
            if constexpr(std::is_same_v<T,float16>){
                div_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __half*>(a.data()),static_cast<const __half*>(b.data()), static_cast<__half*>(dst.data()), a.numel());
            }else if constexpr(std::is_same_v<T,bfloat16>){
                div_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const __nv_bfloat16*>(a.data()),static_cast<const __nv_bfloat16*>(b.data()), static_cast<__nv_bfloat16*>(dst.data()), a.numel());
            }else{
                div_cuda<<<blocks,threads,0,ctx_impl->stream()>>>(static_cast<const T*>(a.data()),static_cast<const T*>(b.data()), static_cast<T*>(dst.data()), a.numel());
            }
        });
        ctx_impl->wait(); 
        return;
