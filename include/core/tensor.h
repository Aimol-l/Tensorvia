...
struct Metadata {
    ...
    via::Device device;                // 记录tensor数据所在后端设备
    via::DataType dtype;               // 记录元素数据类型
...
public:
    ...
    Tensor clone() const;
    via::Device device() const;
    via::DataType dtype() const;

    void to_type(via::DataType);
    Tensor to_type_(via::DataType);
    ...
    void set_impl(std::shared_ptr<TensorImpl> impl,via::DataType new_dtype);
    ...
    Tensor(std::vector<int64_t> shape,via::DataType dtype);
    Tensor(std::initializer_list<int64_t> shape,via::DataType dtype);
    Tensor(std::vector<int64_t> shape,via::DataType dtype,via::Device device);
    Tensor(std::initializer_list<int64_t> shape,via::DataType dtype,via::Device device);
    Tensor(void* ptr, std::vector<int64_t> shape,via::DataType dtype,via::Device device);
    Tensor(void* ptr,std::initializer_list<int64_t> shape,via::DataType dtype,via::Device device);// 深拷贝ptr的内容
    template<typename T>
    ...
    // 常用静态函数
    static Tensor Zeros(std::initializer_list<int64_t> shape, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Zeros(std::vector<int64_t> shape, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Ones(std::initializer_list<int64_t> shape, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Ones(std::vector<int64_t> shape, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Fill(std::initializer_list<int64_t> shape, float value, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Fill(std::vector<int64_t> shape, float value, via::DataType dtype);
    static Tensor Random(std::initializer_list<int64_t> shape, float min = 0.0f, float max = 1.0f, via::DataType dtype = via::DataType::FLOAT32);
    static Tensor Random(std::vector<int64_t> shape, float min = 0.0f, float max = 1.0f, via::DataType dtype = via::DataType::FLOAT32);
};
