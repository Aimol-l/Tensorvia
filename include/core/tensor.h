...
public:
    ...
    virtual void reshape(std::vector<int>& newshape)=0;
    virtual void reshape(std::initializer_list<int> newshape)=0;
...
public:
    ...
    Tensor(std::initializer_list<int> shape,DataType dtype);
    ...
    void reshape(std::initializer_list<int> newshape);
    ...
    // 元素访问
    template<typename T>
    T at(std::initializer_list<int> idxs);

    template<typename T>
    T operator[](std::initializer_list<int> idxs);

    // 常用静态函数
    static Tensor Zeros(std::initializer_list<int> shape, DataType dtype = DataType::FLOAT32);
    static Tensor Zeros(std::vector<int> shape, DataType dtype = DataType::FLOAT32);
    static Tensor Ones(std::initializer_list<int> shape, DataType dtype = DataType::FLOAT32);
    static Tensor Ones(std::vector<int> shape, DataType dtype = DataType::FLOAT32);
    static Tensor Fill(std::initializer_list<int> shape, float value, DataType dtype = DataType::FLOAT32);
    static Tensor Fill(std::vector<int> shape, float value, DataType dtype);
    static Tensor Random(std::initializer_list<int> shape, float min = 0.0f, float max = 1.0f, DataType dtype = DataType::FLOAT32);
    static Tensor Random(std::vector<int> shape, float min = 0.0f, float max = 1.0f, DataType dtype = DataType::FLOAT32);
};
